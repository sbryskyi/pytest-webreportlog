...
def parse_size(text: str | int) -> int:
    """Parse a human-readable size into bytes (1024-based, matching format_size).

    Accepts e.g. "2GB", "500 MB", "1.5G", "2048", or a plain int. Units are
    case-insensitive and the trailing 'B'/'iB' is optional.

    Raises:
        ValueError: if the text cannot be parsed as a size.
    """
...
def format_size(num_bytes: int | None) -> str:
    ...
    Returns 'n/a' when the size is unknown (e.g. an in-memory database).
    """
...
def update_timestamp_bounds(
    ...
    if start is not None and (new_min is None or start < new_min):
        new_min = start

    if stop is not None and (new_max is None or stop > new_max):
        new_max = stop
